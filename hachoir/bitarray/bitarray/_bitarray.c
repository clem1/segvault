...
static PyObject *
bitarray_shift(bitarrayobject *self, PyObject *args)
{
    idx_t i = -1;

    if (!PyArg_ParseTuple(args, "|L:shift", &i))
        return NULL;

    if (self->nbits == 0) {
        /* special case -- most common failure cause */
        PyErr_SetString(PyExc_IndexError, "shift from empty bitarray");
        return NULL;
    }

    if (i < 0 || i >= self->nbits) {
        PyErr_SetString(PyExc_IndexError, "shift index out of range");
        return NULL;
    }

    setbit(self, i, !GETBIT(self, i));

    Py_RETURN_NONE;
}

PyDoc_STRVAR(shift_doc,
"shift(i)\n\
\n\
Shift the i-th element of the bitarray.");
...
bitarray_methods[] = {
    ...
    {"pop",          (PyCFunction) bitarray_pop,         METH_VARARGS,
     pop_doc},
    {"shift",          (PyCFunction) bitarray_shift,     METH_VARARGS,
     shift_doc},
